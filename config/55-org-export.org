...
markdown, the LaTeX and memoir classes, beamer, minted, ox-hugo
...
that capture disappear until the first export of the session.  It appends with
=add-to-list ... t=, so it lands after the personal templates that =40-org= sets
with =setq=.
