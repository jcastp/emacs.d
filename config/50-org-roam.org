...
utility functions -- and the personal capture templates, which used to sit ~350
lines away in another file from the base templates they extend.

Shared by both environments against the same database; the directory comes from
=my-roam-dir= in =05-identity.org=.
