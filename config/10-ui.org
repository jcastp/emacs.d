...
=my/set-hl-line-face=, applied whenever a theme is enabled.
...
Two themes are skipped: =user= (customizations, which =17-org-style= writes
into) and =fontaine= (fonts only).  Neither changes the palette.
