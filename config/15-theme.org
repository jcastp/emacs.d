...
=variable-pitch= are set only there.  They used to be set in the =user= theme
as well, and the =user= theme outranks fontaine's every time a theme is
enabled -- so the startup rotation, =C-c 0= and org-scribe's mode switches all
silently reset the preset to Aporetic.
...
#+begin_src emacs-lisp
  (when my-workenvironment-p
    (with-eval-after-load 'consult
      (consult-theme 'modus-operandi-deuteranopia)
