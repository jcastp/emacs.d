...
** tab-bar workspaces
Built-in since Emacs 27. Replaces =perspective=.
Session save/restore uses =window-state-get/put= plus the list of open files.

Note: existing =perspectives/= files are in perspective's proprietary format and
cannot be read by the new loader. Recreate sessions once with =C-c w s= after
...
#+begin_src emacs-lisp
  (use-package tab-bar
    ...
    (tab-bar-mode 1)
    :bind
    ("C-c w w" . tab-bar-switch-to-tab)
    ("C-c w n" . tab-bar-new-tab)
    ("C-c w r" . tab-bar-rename-tab)
    ("C-c w k" . tab-bar-close-tab)
    ("C-x C-b" . ibuffer))

  (defvar my-tab-session-dir (expand-file-name "tab-sessions/" my-data-dir)
    "Directory for saved tab sessions.")

  (defun my/tab-session-save (name)
    "Save current window layout and open files as a named tab session."
    (interactive "sSession name: ")
    (let ((file (expand-file-name name my-tab-session-dir))
          (buffers (delq nil (mapcar #'buffer-file-name (buffer-list))))
          (state (window-state-get (frame-root-window))))
      (make-directory my-tab-session-dir t)
      (with-temp-file file
        (print (list :buffers buffers :window-state state) (current-buffer)))
      (message "Saved session: %s" name)))

  (defun my/tab-session-load (file)
    "Load a named tab session from FILE, opening a new tab if needed."
    (interactive (list (read-file-name "Load session: " my-tab-session-dir)))
    (let* ((data (with-temp-buffer
                   (insert-file-contents file)
                   (read (current-buffer))))
           (buffers (plist-get data :buffers))
           (state   (plist-get data :window-state))
           (tab-name (file-name-base file)))
      (if (member tab-name (mapcar (lambda (tab) (alist-get 'name tab)) (tab-bar-tabs)))
      ...
      (message "Loaded session: %s" tab-name)))

  (global-set-key (kbd "C-c w l") #'my/tab-session-load)
  (global-set-key (kbd "C-c w s") #'my/tab-session-save)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (setq org-todo-keyword-faces
        '(("TODO"      . my/org-todo-active)
          ...
          ("BLOCKED"   . my/org-todo-blocked)
          ("HOLD"      . my/org-todo-blocked)
...
** tree-sitter config
Tree-sitter is built into Emacs 29+. The external =tree-sitter= and
=tree-sitter-langs= packages are not needed — use =python-ts-mode= and
other =*-ts-mode= modes directly.

ts-fold depends on the external tree-sitter package, so it is disabled.
Native code folding alternative: =hs-minor-mode= (built-in).

#+begin_src emacs-lisp :tangle no
  ;; ts-fold disabled: depends on external tree-sitter which was removed.
  ;; Alternative: (add-hook 'prog-mode-hook #'hs-minor-mode)
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/key-prefix-direct-access-map
    ...
    "c" (cons "personal config"
  	     (lambda ()
  	       (interactive)
  	       (my/tab-session-load (expand-file-name "config" my-tab-session-dir))))
    "k" (cons "emacs keys"
  	(lambda ()
  	  (interactive)
    ...
    "t" (cons "tasks file"
  	   (lambda ()
  	     (interactive)
  	     (my/tab-session-load (expand-file-name "tareas" my-tab-session-dir))))
    "e" (cons "escritura tab"
  	      (lambda ()
  		(interactive)
  		(my/tab-session-load (expand-file-name "escritura" my-tab-session-dir)))))
