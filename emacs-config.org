...
current buffer’s directory."
  (interactive)
  ;; 1️⃣ Determine the project root.
  (let* ((proj   (or (project-current)
         ...
         ;; 2️⃣ Gather all *.org* files under that root.
         (org-files
          (directory-files-recursively
           root "\\.org\\'" nil
           ;; Optional: ignore hidden directories like .git, .svn, etc.
           (lambda (dir) (not (string-match-p "/\\.[^/]+\\'" dir))))))
    ;; 3️⃣ Temporarily bind `org-agenda-files' to that list and run the agenda.
    (let ((org-agenda-files org-files))
      (org-agenda))))   ;; “a” = the default agenda view 
...
- For general purpose files or directories, it uses the "Notas" header on that file.
- For writing projects, where the =notas/notas.org= file exists, it will use that file.
#+begin_src emacs-lisp
  (defun my/capture-to-this-buffer ()
    "Capture a note to a dedicated `notas/notas.org` file if it exists.

  If a file named `notas/notas.org` is found in a sub‑directory of the
  current buffer’s directory, the capture is stored there under the
  headline \"Notes\".  When the file cannot be located, the capture is
  saved to the current buffer’s file (also under the headline
  \"Notes\")."
    (interactive)
    ;; 1️⃣ Make sure we are in an Org buffer.
    (unless (derived-mode-p 'org-mode)
      (user-error "Can’t capture to a non‑Org buffer"))

    ;; 2️⃣ Resolve the two possible target files.
    (let* ((cur-dir   (or (file-name-directory (buffer-file-name))
                          default-directory))   ; safety net
           (fallback  (buffer-file-name))                ; current file
           (alt-path  (expand-file-name "notas/notas.org" cur-dir)))

      ;; 3️⃣ Choose the destination that actually exists.
      (let ((target (if (file-exists-p alt-path) alt-path fallback)))
        ;; 4️⃣ Build a temporary capture template that points at TARGET.
        (let ((org-capture-templates
               `(("t" "Todo"
                  entry
                  (file+headline ,target "Notas")
                  "** TODO %?"))))
          ;; 5️⃣ Run the capture UI.
          (org-capture)))))
...
#+begin_src emacs-lisp
  ...
  ;; ------------------------------------------------------------
  (defvar-local my-editing--saved-config nil
    "Window configuration saved before `my-editing-mode' was enabled.")
  (defvar-local my-editing--saved-theme nil
    "List of themes that were enabled before `my-editing-mode' was turned on.")
  (defvar-local my-editing--saved-fill-column-width nil
    "Value of `visual-fill-column-width' before the mode was enabled.")
  (defvar-local my-editing--saved-fontaine-preset nil
  ...
  (defun my-editing--setup ()
    ...
    (let* ((left-perc   0.25)
           ...
           (src-file
            (or (buffer-file-name)
                (error "Current buffer is not visiting a file; cannot enable `my-editing-mode'")))
           (notes-file (my/file-notes-filename src-file))
           (frame-w    (frame-width))
           (right-w    (my/window-perc right-perc))
           (left-w     (my/window-perc left-perc)))

      ;; ----- Save visual state -----
      (setq my-editing--saved-config (current-window-configuration)
  	  my-editing--saved-theme
  	  (when (boundp 'custom-enabled-themes) custom-enabled-themes)
  	  my-editing--saved-fill-column-width visual-fill-column-width
  	  ;; NEW: store the *actual* active preset
  	  my-editing--saved-fontaine-preset
  	  (when (fboundp 'fontaine-current-preset)
  ...
  ;; ------------------------------------------------------------
  (defun my-editing--teardown ()
    "Restore the previous window configuration *and* visual settings."
    ;; Windows
    (when (window-configuration-p my-editing--saved-config)
      (set-window-configuration my-editing--saved-config))

    ;; Themes
    (when (and (boundp 'custom-enabled-themes) my-editing--saved-theme)
      (dolist (th custom-enabled-themes)
        (unless (member th my-editing--saved-theme)
          (disable-theme th)))
      (dolist (th my-editing--saved-theme)
        (unless (member th custom-enabled-themes)
          (load-theme th t))))

    ;; visual‑fill‑column‑width
    (when (boundp 'visual-fill-column-width)
      (setq visual-fill-column-width my-editing--saved-fill-column-width))

    ;; Fontaine preset
    (when (and (boundp 'fontaine-set-preset)
               my-editing--saved-fontaine-preset)
      (fontaine-set-preset my-editing--saved-fontaine-preset))

    ;; Clean up saved vars
    (setq my-editing--saved-config nil
          my-editing--saved-theme nil
          my-editing--saved-fill-column-width nil
          my-editing--saved-fontaine-preset nil))
  ...
  ;; ------------------------------------------------------------
  ;; Optional convenience keybinding
  ;; ------------------------------------------------------------
  ;; (global-set-key (kbd "C-c d e") #'my-editing-mode)
#+end_src
      ...
      (save-excursion
        ...
        (while (re-search-forward "\\w+" end t)
          (unless (or (org-in-commented-line)
    		  ...
    		  (org-in-heading-p)
                      (org-at-property-p)
  		    ;; this could be not very good if we use the quote in a book
  		    (org-in-block-p))
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/key-prefix-writing-map
    ...
    "e" #'my-editing-mode
    "w" #'my/ews-org-count-words
    "c" #'my/org-count-words
    ;; writing exercises
