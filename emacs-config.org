...
- =cape-dict= for words that have never been typed anywhere, drawn
  from the =hunspell= English and Spanish word lists (the same
  dictionaries used for spellchecking, see the spelling section
  above). Hunspell's =.dic= files carry affix flags
  (e.g. =dog/S=) that cape-dict must not show as candidates, so a
  plain, flag-stripped word list is generated once into
  =my-data-dir= and reused on subsequent starts.
...
** CANCELED C/C++
CLOSED: [2026-06-19 vie 09:12]
:LOGBOOK:
- State "CANCELED"   from              [2026-06-19 vie 09:12]
:END:
