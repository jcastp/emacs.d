...
#+begin_src emacs-lisp
  (custom-theme-set-faces
   ...
   '(default
     ((t (:family "Iosevka Comfy" :height 120 :weight Light)))
     )
   )
#+end_src
...
#+begin_src emacs-lisp
  ...
  ^Resize^
  ^^^^^^-------------------------------------------------------
  _C-f_: shrink horizontally
  _C-b_: enlarge horizontally
  _C-p_: shrink vertical
  _C-n_: enlarge vertical
  "
       ...
       ;; ("<up>" enlarge-window)
       ;; ("<down>" shrink-window)
       ("C-f" shrink-window-horizontally)
       ("C-b" enlarge-window-horizontally)
       ("C-p" enlarge-window)
       ("C-n" shrink-window)
...
Removing this one, and using the one from the source code below:
#+begin_src emacs-lisp :tangle no
  (use-package nov
    :straight t
    :config
...
** TODO bibliography - config
All the config needed to manage bibliographies and references.

*** bibtex
The main files are the .bib files, where all the information about the reading material is stored.
...
#+begin_src emacs-lisp
  (use-package bibtex
    ...
    (bibtex-file-path "~/Nextcloud/config/.emacs.d/bibliography/")
    )
#+end_src

*** biblio
Biblio to automatically import books or articles from different repositories.
#+begin_src emacs-lisp
  (use-package biblio
    :straight t)
#+end_src
...
*** citar
Citar allows to insert bibliographic references in our texts.
#+begin_src emacs-lisp
  (use-package citar
    :straight t
    :config
    (setq citar-bibliography '("~/Nextcloud/config/.emacs.d/bibliography/biblio.bib"))
    :custom
...
*** TODO citar-org-roam integration
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;;   :straight (random-table :host github :repo "jeremyf/random-table.el"))
#+end_src  
