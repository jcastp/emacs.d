...
Removed corfu and cape. TAB completes via native =completion-at-point=
(=tab-always-indent 'complete= is set in the minibuffer section above).
Candidates appear in the =*Completions*= buffer, navigated with vertico.

#+begin_src emacs-lisp
  ;; dabbrev-capf is built-in since Emacs 28 — replaces cape-dabbrev
...
** CANCELED emacs-rotate
CLOSED: [2026-06-18 jue 10:54]
:LOGBOOK:
- State "CANCELED"   from              [2026-06-18 jue 10:54]
:END:
...
Replaced flycheck — eglot uses flymake, so flycheck was redundant.
...
These values allow for automating the treesitter grammars. =treesit-enabled-modes
t= auto-remaps legacy modes (=c++-mode= → =c++-ts-mode=, etc.) whenever the
matching grammar is available, and =treesit-auto-install-grammar 'always=
installs a missing grammar automatically the first time such a buffer opens —
no =y/n= prompt. (The valid values are =never=/=always=/=ask=/=ask-dir=; a plain
=t= is *not* valid and silently falls back to prompting, which is why grammars
weren't installing.)

Installing a grammar compiles it from source, so the host also needs =git= and a
C/C++ compiler (see the C/C++ heading below for the full OS package list).
#+begin_src emacs-lisp
...
=eglot-server-programs= already maps =c-ts-mode=/=c++-ts-mode= to =clangd= by
default, so C/C++ only needs the =eglot-ensure= hook below, nothing added to
...
Everything here drives external tools that must be on =PATH=; none of them are
Emacs packages. On a Debian/Ubuntu host:

#+begin_src sh :tangle no
  sudo apt install clangd gdb git build-essential
#+end_src

| package         | needed by              | why                                                        |
|-----------------+------------------------+------------------------------------------------------------|
| =clangd=        | eglot                  | LSP server: completion, diagnostics, =eglot-format-buffer= |
| =gdb=           | dape                   | debug adapter backend for =M-x dape=                       |
| =git=           | treesit grammar build  | clones the tree-sitter-cpp / -c grammar repos              |
| =build-essential= | treesit grammar build | =gcc=/=g++=/=make= to compile the grammar =.so= files      |

=clangd= is what eglot talks to for completion, diagnostics, and formatting;
flymake picks up eglot's diagnostics automatically, since it's hooked to
=prog-mode= above. The grammar build (=git= + a compiler) only runs the first
time a C/C++ buffer opens, because =treesit-auto-install-grammar 'always=
compiles =libtree-sitter-cpp.so= / =libtree-sitter-c.so= into
=~/.emacs.d/tree-sitter/=.

*** indentation style
Until the cpp grammar is installed you fall back to =c-mode=/=c++-mode= (legacy
cc-mode); once =treesit-auto-install-grammar 'always= has pulled the grammar you
get =c-ts-mode=/=c++-ts-mode=. The two use different indentation engines, so set
*both* the tree-sitter and the legacy indent variables to keep a consistent 4-space
=k&r= style either way.

Crucially, =indent-tabs-mode= defaults to =t=, which makes both engines insert
literal TAB characters once the indentation passes one =tab-width= — producing the
mixed tabs/spaces you saw (a stray =\t= in front of nested lines). Force spaces in
all four C/C++ modes so a level is always 4 spaces, never a tab.
#+begin_src emacs-lisp
  ...
  (defun my/c-indent-setup ()
    "Indent C/C++ with spaces only, never literal tab characters."
    (setq indent-tabs-mode nil))

  (dolist (hook '(c-mode-hook c++-mode-hook c-ts-mode-hook c++-ts-mode-hook))
    (add-hook hook #'my/c-indent-setup))
#+end_src

*** force tree-sitter modes for C/C++ extensions
#+begin_src emacs-lisp
  (dolist (ext '("\\.cpp\\'" "\\.cc\\'" "\\.cxx\\'" "\\.hpp\\'" "\\.hh\\'"))
    (add-to-list 'auto-mode-alist (cons ext 'c++-ts-mode)))
  ;; .h is ambiguous between C and C++; c-or-c++-ts-mode sniffs the
  ;; buffer content instead of always assuming C.
  (add-to-list 'auto-mode-alist '("\\.h\\'" . c-or-c++-ts-mode))
#+end_src

*** build
Compiles the current project with =project-compile=, prompting for the
command the first time (e.g. =make= or =cmake --build build=) and reusing it
after that.
#+begin_src emacs-lisp
...
*** dape - debugging
Debug Adapter Protocol client, the eglot/flymake-style counterpart for
debugging. Needs =gdb= on PATH (=sudo apt install gdb=). Run =M-x dape=,
pick the =gdb= launch config, and give it the path to the compiled binary.
#+begin_src emacs-lisp
  (use-package dape
    :ensure t
    :config
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/key-prefix-direct-access-map
    ...
    "t" (cons "tasks file"
  	   (lambda ()
  	     (interactive)
  	     (find-file "~/Nextcloud/agenda/tasks.org")))
    "e" (cons "escritura tab"
  	      (lambda ()
  		(interactive)
  		(find-file "~/Nextcloud/escritura/historias/novela"))))
  ...
  (defvar-keymap my/key-prefix-map
    ...
    "p" (when (fboundp 'my/open-report)
  	  (cons "open report" #'my/open-report))
    "i" #'tempel-insert
    ...
    "a" (when (boundp 'my/key-prefix-ai-map)
  	(cons "AI" my/key-prefix-ai-map))
