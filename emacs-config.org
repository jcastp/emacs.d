...
These values allow for automating the treesitter grammars.
#+begin_src emacs-lisp
...
** C/C++
Requires =clangd= on PATH (=sudo apt install clangd=) — that's what eglot
talks to for completion, diagnostics, and formatting. flymake picks up
eglot's diagnostics automatically, since it's already hooked to =prog-mode=
above.

*** indentation style
=treesit-auto-install-grammar=/=treesit-enabled-modes= above don't actually
remap =.c=/=.cpp= files to the tree-sitter modes until the cpp grammar is
installed, so =c-mode=/=c++-mode= (legacy cc-mode) is what you'll actually
get on a fresh machine — set both the tree-sitter and the legacy indent
variables so indentation is correct either way.
#+begin_src emacs-lisp
