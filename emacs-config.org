...
** tree-sitter config
Tree-Sitter is built into Emacs 29+. We use =treesit-auto= to manage grammars
and automatically remap major modes to their =*-ts-mode= variants.
Note: =emacs-lisp-ts-mode= does not exist yet upstream.
