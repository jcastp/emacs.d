...
In order to make this work, we need to use the =org-babel-tangle= command, bound to =C-c C-v C-t=. This will generate the different files needed, shown in the structure below:
...
Only evaluated if in my home env, and my encrypted directory is mounted.
Auth information is stored in my encrypted directory.
...
#+begin_src emacs-lisp :tangle "modules/my-work.el" :mkdirp
;;  (setq org-refile-targets (quote (("trabajo.org" :maxlevel . 9))))
