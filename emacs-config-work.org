...
* Persist minibuffer history (work)
Savehist is only enabled in the personal config, so the work environment had no
persisted history.  Enable it here, with a work-specific file, and use it to
remember the last direct report opened via =my/open-report=.

#+begin_src emacs-lisp
  (setq savehist-file (expand-file-name "savehist-work" my-data-dir))
  (savehist-mode 1)
