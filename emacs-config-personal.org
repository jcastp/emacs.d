...
#+begin_src emacs-lisp
  ...
  ;; colors used for the highlighted current line, per theme kind
  ...
  (defun my/set-hl-line-face (dark-p)
    "Set the `hl-line' face colors for a dark or clear theme, based on DARK-P."
    (if dark-p
        (set-face-attribute 'hl-line nil
                             :background my-hl-line-dark-background
                             ;; :foreground my-hl-line-dark-foreground
  			   )
      (set-face-attribute 'hl-line nil
                           :background my-hl-line-clear-background
                           ;; :foreground my-hl-line-clear-foreground
  			 )))
...
** Open the writing environment
One-key entry point to start a writing session, to reduce the friction of
getting started. It prompts to pick a project among the subdirectories of
=my-writing-root=, but offers the *last project used* as the default: press RET
to reopen it, or type to choose another. The last choice is remembered across
sessions through savehist (see =my/last-writing-project= in the savehist config).
...
** TODO org-scribe config
I want to test my org-scribe package, so I'm using this config for that.
...
*This code is test for conditional load, depending on the availability of the local directory or the git repo*:
...
* Testing
Staging area for code not yet promoted to the main config.
